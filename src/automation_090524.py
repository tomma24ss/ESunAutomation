...
class SolarBoilerAutomation:
    ...
    def check_conditions_Heatpump(self):
        ...
        try:
            ...
            if all(wattage < self.HEATPUMP_TOGGLE_WATTAGE for wattage in wattage_values):
                self.logger.debug(f"Heatpump - last 10 minutes under {self.HEATPUMP_TOGGLE_WATTAGE}W => go to time check")
                actief = True
            elif all(wattage > self.HEATPUMP_TOGGLE_WATTAGE for wattage in wattage_values):
                self.logger.debug(f"Heatpump - last 10 minutes above {self.HEATPUMP_TOGGLE_WATTAGE}W => skip time check on go to boiler")
                actief = False
                return actief
    ...
    def activate_relay_dagnacht(self):
        try:
            GPIO.output(self.relay_pin_dagnacht, GPIO.LOW)  # Activate the relay
        except Exception as e:
            self.logger.error("An error occurred while activating relay_dagnacht: " + str(e))   
    def deactivate_relay_dagnacht(self):
        try:
            GPIO.output(self.relay_pin_dagnacht, GPIO.HIGH)  # Deactivate the relay
        except Exception as e:
            self.logger.error("An error occurred while deactivating relay_dagnacht: " + str(e))
    def activate_relay_ac(self):
        try:
            GPIO.output(self.relay_pin_ac, GPIO.LOW)  # Activate the relay
        except Exception as e:
            self.logger.error("An error occurred while activating relay_ac: " + str(e))
    def deactivate_relay_ac(self):
        try:
            GPIO.output(self.relay_pin_ac, GPIO.HIGH)  # Deactivate the relay
        except Exception as e:
            self.logger.error("An error occurred while deactivating relay_ac: " + str(e))
    ...
    def run(self):  
        try:
            ...
            if self.sunsetconditions.check_conditions_dagnacht():
                self.logger.debug("Dagnacht: Pin {} actief (niet draaien)".format(self.relay_pin_dagnacht))
                if str(self.OK_TO_SWITCH) == "True":
                    self.activate_relay_dagnacht()
                    self.logger.debug("Dagnacht: Pin {} aan (niet draaien)".format(self.relay_pin_dagnacht))
            else:
                self.logger.debug("Dagnacht: Pin {} niet actief (draaien)".format(self.relay_pin_dagnacht))
                if str(self.OK_TO_SWITCH) == "True":
                    self.deactivate_relay_dagnacht()
                    self.logger.debug("Dagnacht: Pin {} uit (draaien)".format(self.relay_pin_dagnacht))
            if self.check_conditions_ac():
                self.logger.debug("AC: Pin {} actief (niet draaien)".format(self.relay_pin_ac))
                if str(self.OK_TO_SWITCH) == "True":
                    self.activate_relay_ac()
                    self.logger.debug("AC: Pin {} aan (niet draaien)".format(self.relay_pin_ac))
            else:
                self.logger.debug("AC: Pin {} niet actief (draaien)".format(self.relay_pin_ac))
                if str(self.OK_TO_SWITCH) == "True":
                    self.deactivate_relay_ac()
                    self.logger.debug("AC: Pin {} uit (draaien)".format(self.relay_pin_ac))
