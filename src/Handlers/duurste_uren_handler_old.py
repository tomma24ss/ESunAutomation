...
class DuursteUrenHandler:
    ...
    def is_duurste_uren(self):
        try:
            ...
            # Increment the hour if the minute is greater than 0
            if current_minute > 0:
                current_hour += 1

                # If the hour is 24, keep it as 24 (23:XX should become 24)
                if current_hour == 24:
                    current_hour = 24
                
                # If the hour exceeds 24 (like 00:XX becomes 1)
                elif current_hour == 25:
                    current_hour = 1

            # Update the now variable with the calculated current hour
            now = current_hour
